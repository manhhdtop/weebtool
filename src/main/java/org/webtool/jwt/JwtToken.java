...
public class JwtToken
{
	...
	public static Optional<User> decodeJWT(String token)
	{
		try
		{
			...
			return Optional.of(DBUtil.getUser(username));
		}
		catch (JWTVerificationException | ClassNotFoundException | NullPointerException
		        | SQLException e)
		{
	...
	{
		JWTVerifier verifier = JWT.require(algorithm).acceptExpiresAt(0).withIssuer("issuer")
		        .build();
